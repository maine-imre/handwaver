...
To fold the net of an 8-cell, the center cube is fixed.  The six adjacent cubes are rotated around a plane that is orthagonal to the face of the cube and colinear with the w-axis (if the net is constructed such that w is fixed for all verticies of the cubes), by $\frac{\pi}{4}$.  The remaining cube is rotated with respect to its adjacent cube by $\frac{\pi}{4}$ or by $\frac{\pi}{2}$ with respect to it's origional position, with the rotation centerd on its shared face (note that this face is also moving during the rotaiton).
