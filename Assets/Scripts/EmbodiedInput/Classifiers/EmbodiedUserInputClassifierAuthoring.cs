...
namespace IMRE.EmbodiedUserInput
{
    public class EmbodiedUserInputClassifierAuthoring : MonoBehaviour
    {
        ...
        {
            BodyInput currBodyInput = BodyInputDataSystem.bodyInput;
            int amount = 13;
            entities = new NativeArray<Entity>(amount, Allocator.Temp);
            manager.Instantiate(classifierPrefab, entities);
            for (int i = 0; i < amount; i++)
            {
                manager.SetComponentData(entities[i], currBodyInput);
            }
        ...
        void updateClassifierBatch()
        {
            foreach (Entity entity in entities)
            {
                manager.SetComponentData(entity, BodyInputDataSystem.bodyInput);
            }
        }

        private void FixedUpdate()
        {
            updateClassifierBatch();
        }
    }
}