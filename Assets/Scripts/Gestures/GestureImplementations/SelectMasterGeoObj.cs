...
{
    public class SelectMasterGeoObj : PointAtGesture
    {
        ...
        private MasterGeoObj bestObject(Hand hand)
        {
            ...
            List<MasterGeoObj> eligibleMGO = FindObjectsOfType<MasterGeoObj>().Where(mgo => checkEligible(mgo, hand, outerRadius)).ToList();
            ...
            if (closestObj != null && closestObj2 != null)
            {
                ...
                if (closestRatio < aboutEqual && closestRatio > muchCloser)
                {
                    ...
                    eligibleMGO = eligibleMGO.Where(mgo => checkEligible(mgo, hand, shortestDist * aboutEqual)).ToList();
                    //filter by dimension
